...
7. [HW3-4: Rainbow DQN - Integrated Optimization](#hw3-4-rainbow-dqn---integrated-optimization)
...
```
RL-HW3/
├── Rainbow DQN.ipynb                  # HW3-4: Integrated Rainbow DQN implementation
├── (HW3-4)Rainbow_DQN_Technical_Analysis_Report.pdf # Comprehensive technical analysis
...
### 🛠️ Key Integrated Components
- **Double DQN**: Decouples action selection from evaluation to mitigate overestimation.
- **Dueling Network**: Separates state value $V(s)$ and action advantage $A(s, a)$.
- **Prioritized Experience Replay (PER)**: Focuses training on "difficult" transitions with high TD-error.
- **Multi-step Learning**: Uses a 3-step return to accelerate reward propagation.
- **Noisy Networks**: Replaces $\epsilon$-greedy with learnable parameter noise for robust exploration.

### 📝 Technical Discussion: The "Loss Paradox"
In the 4x4 Gridworld (Random Mode), Rainbow DQN exhibited higher and more volatile loss curves compared to simpler variants like Soft Update DQN. This is a result of:
1. **PER** focusing on high-error samples.
2. **Noisy Nets** maintaining persistent exploration via weight perturbations.
3. **N-step** returns increasing variance in target estimation.

Despite the "noisier" training process, these mechanisms are critical for scaling to high-dimensional tasks such as **UAV Autonomous Navigation**, where sparse rewards and complex state spaces demand the advanced exploration and prioritization provided by Rainbow DQN.
...
| HW3-4 | Rainbow DQN | PER + Noisy + N-step | **100.0%** | High/Oscillating |
...
4. **Rainbow DQN**: Hessel, M., Modayil, J., van Hasselt, H., et al. (2018). *"Rainbow: Combining Improvements in Deep Reinforcement Learning."* AAAI Conference on Artificial Intelligence.

5. **Prioritized Experience Replay**: Schaul, T., Quan, J., Antonoglou, I., & Silver, D. (2016). *"Prioritized Experience Replay."* ICLR.

6. **Noisy Networks**: Fortunato, M., Azar, M. G., et al. (2018). *"Noisy Networks for Exploration."* ICLR.

7. **Distributional RL**: Bellemare, M. G., Dabney, W., & Munos, R. (2017). *"A Distributional Perspective on Reinforcement Learning."* ICML.
